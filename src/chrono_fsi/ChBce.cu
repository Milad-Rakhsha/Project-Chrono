        ...
        for (int y = -1; y <= 1; y++) {
            for (int x = -1; x <= 1; x++) {
                ...
                BCE_modification_Share(sumVW, sumWAll, sumRhoRW, sumPW, sumWFluid, isAffectedV, isAffectedP,
                                       neighbourPos, posRadA, sortedPosRad, sortedVelMas, sortedRhoPreMu, cellStart,
                                       cellEnd);
            }
        }
    }
...
__global__ void Calc_Markers_TorquesD(Real3* torqueMarkersD,
    ...
    torqueMarkersD[index] =
        paramsD.markerMass * cross(dist3, mR3(derivVelRhoD[rigidMarkerIndex]));  // paramsD.markerMass
                                                                                 // is multiplied to
                                                                                 // convert
...
void ChBce::Rigid_Forces_Torques(SphMarkerDataD* sphMarkersD, FsiBodiesDataD* fsiBodiesD) {
    ...
    Calc_Markers_TorquesD<<<nBlocks_numRigid_SphMarkers, nThreads_SphMarkers>>>(
        mR3CAST(torqueMarkersD), mR4CAST(fsiGeneralData->derivVelRhoD), mR4CAST(sphMarkersD->posRadD),
        U1CAST(fsiGeneralData->rigidIdentifierD), mR3CAST(fsiBodiesD->posRigid_fsiBodies_D));
    cudaThreadSynchronize();
    cudaCheckError();
    ...
    (void)thrust::reduce_by_key(fsiGeneralData->rigidIdentifierD.begin(), fsiGeneralData->rigidIdentifierD.end(),
                                ...
                                fsiGeneralData->rigid_FSI_TorquesD.begin(), binary_pred, thrust::plus<Real3>());
}
////////////////////////////////////////////////////////////////////////////////////////////////////////
