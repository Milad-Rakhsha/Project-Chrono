...
//	printf("Job was submittet at date/time: %s\n", asctime(timeinfo));
  ...
  outMarker << "x, y, z, vx, vy, vz, rho, p, mu, type, index\n";
  for (int i = 0; i < mPosRad.size(); i++) {
    Real3 p = mPosRad[i];
    Real3 vM = mVelMas[i];
    Real4 rPMtype = mRhoPresMu[i];
    uint index = bodyIndex[i];
    outMarker << p.x << ", " << p.y << ", " << p.z << ", " << vM.x << ", " << vM.y << ", " << vM.z << ", " << rPMtype.x
              << ", " << rPMtype.y << ", " << rPMtype.z << ", " << rPMtype.w << ", " << index << ",\n ";
  }
  outMarker.close();
...
void CheckPointMarkers_Read(bool shouldIRead,
  ...
  Real4 rPMtype;
  uint index;
  inMarker >> p.x >> ddCh >> p.y >> ddCh >> p.z >> ddCh >> vM.x >> ddCh >> vM.y >> ddCh >> vM.z >> ddCh >> rPMtype.x >>
      ddCh >> rPMtype.y >> ddCh >> rPMtype.z >> ddCh >> rPMtype.w >> ddCh >> index >> ddCh;
  while (inMarker.good()) {
    mPosRad.push_back(p);
    ...
    inMarker >> p.x >> ddCh >> p.y >> ddCh >> p.z >> ddCh >> vM.x >> ddCh >> vM.y >> ddCh >> vM.z >> ddCh >>
        rPMtype.x >> ddCh >> rPMtype.y >> ddCh >> rPMtype.z >> ddCh >> rPMtype.w >> ddCh >> index >> ddCh;
  }
  inMarker.close();
