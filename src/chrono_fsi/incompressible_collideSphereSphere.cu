...
void ForceSPH_implicit(thrust::device_vector<Real3>& posRadD,
  ...
  thrust::device_vector<Real3> m_dSortedVelMas(
      numAllMarkers);  // Store velocities of each particle in the device memory
  thrust::device_vector<Real4> m_dSortedRhoPreMu(
  ...
  thrust::device_vector<uint> mapOriginalToSorted(
      numAllMarkers);                                        // Store mapOriginalToSorted[originalIndex] = sortedIndex
   ...
   */
  reorderDataAndFindCellStart(m_dCellStart, m_dCellEnd, m_dSortedPosRad, m_dSortedVelMas, m_dSortedRhoPreMu,
                              m_dGridMarkerHash, m_dGridMarkerIndex, mapOriginalToSorted, posRadD, velMasD, rhoPresMuD,
...
void Rigid_Forces_Torques(thrust::device_vector<Real3>& rigid_FSI_ForcesD,
  ...
  (void)thrust::reduce_by_key(rigidIdentifierD.begin(), rigidIdentifierD.end(),
                              derivVelRhoD.begin() + numObjects.startRigidMarkers, dummyIdentify.begin(),
  ...
  Calc_Rigid_FSI_ForcesD<<<nBlock_UpdateRigid, nThreads_rigidParticles>>>(mR3CAST(rigid_FSI_ForcesD),
  ...
  (void)thrust::reduce_by_key(rigidIdentifierD.begin(), rigidIdentifierD.end(), torqueMarkersD.begin(),
                              dummyIdentify.begin(), rigid_FSI_TorquesD.begin(), binary_pred, thrust::plus<Real3>());
...
void UpdateRigidMarkersPositionVelocity(thrust::device_vector<Real3>& posRadD,
  ...
  UpdateRigidMarkersPositionVelocityD<<<nBlocks_numRigid_SphMarkers, nThreads_SphMarkers>>>(
      mR3CAST(posRadD), mR3CAST(velMasD), mR3CAST(rigidSPH_MeshPos_LRF_D), U1CAST(rigidIdentifierD), mR3CAST(posRigidD),
      mR4CAST(velMassRigidD), mR3CAST(omegaLRF_D), mR4CAST(qD));
  cudaThreadSynchronize();
  cudaCheckError();
}
//--------------------------------------------------------------------------------------------------------------------------------
void UpdateRigidMarkersPositionVelocity(thrust::device_vector<Real3>& posRadD,
  ...
  UpdateRigidMarkersPositionVelocity(posRadD, velMasD, rigidSPH_MeshPos_LRF_D, rigidIdentifierD, posRigidD, qD,
                                     velMassRigidD, omegaLRF_D, numObjects);
}
//--------------------------------------------------------------------------------------------------------------------------------
...
void IntegrateIISPH(thrust::device_vector<Real4>& derivVelRhoD,
