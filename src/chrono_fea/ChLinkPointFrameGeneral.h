...
#ifndef CHLINKPOINTFRAMEGENERAL_H
#define CHLINKPOINTFRAMEGENERAL_H
...
class ChApiFea ChLinkPointFrameGeneral : public ChLinkBase {
    // Tag needed for class factory in archive (de)serialization:
    CH_FACTORY_TAG(ChLinkPointFrameGeneral)

  private:
    ChVector<> m_react;
    // Indicator of the coordinates that are not constraints, e.g. (0,0,1) for motion in z dir
    ChVector<uint> relax;
  ...
  public:
    ChLinkPointFrameGeneral();
    ChLinkPointFrameGeneral(ChVector<uint> other_relax);
    ChLinkPointFrameGeneral(const ChLinkPointFrameGeneral& other);
    ~ChLinkPointFrameGeneral() {}

    /// "Virtual" copy constructor (covariant return type).
    virtual ChLinkPointFrameGeneral* Clone() const override { return new ChLinkPointFrameGeneral(*this); }

    /// Get the number of scalar variables affected by constraints in this link
    virtual int GetNumCoords() override { return (3 + 7); }

    /// Number of scalar constraints.
    virtual int GetDOC_c() override { return (relax.x + relax.y + relax.z); }
