...
#include "params_test_fsi_implicit_staticTank.h"  //SetupParamsH()
...
int main(int argc, char* argv[]) {
    ...
    Populate_RigidSPH_MeshPos_LRF(rigidIdentifierD, rigidSPH_MeshPos_LRF_D, posRadD, posRigid_fsiBodies_D,
                                  q_fsiBodies_D, referenceArray, numObjects);

    // sync BCE velocity and position with rigid bodies kinematics
    UpdateRigidMarkersPositionVelocity(posRadD, velMasD, rigidSPH_MeshPos_LRF_D, rigidIdentifierD, posRigid_fsiBodies_D,
                                       q_fsiBodies_D, velMassRigid_fsiBodies_D, omegaVelLRF_fsiBodies_D, numObjects,
                                       paramsH);
    ...
    for (int tStep = 0; tStep < stepEnd; tStep++) {
        ...
        DoStepFluid_implicit(posRadD, velMasD, rhoPresMuD,
                             ...
                             posRigid_fsiBodies_D, velMassRigid_fsiBodies_D, accRigid_fsiBodies_D, q_fsiBodies_D,
                             omegaVelLRF_fsiBodies_D, omegaAccLRF_fsiBodies_D,

                             posRigid_fsiBodies_D2, velMassRigid_fsiBodies_D2, accRigid_fsiBodies_D2, q_fsiBodies_D2,
                             omegaVelLRF_fsiBodies_D2, omegaAccLRF_fsiBodies_D2,

                             pos_ChSystemBackupH, vel_ChSystemBackupH, acc_ChSystemBackupH, quat_ChSystemBackupH,
                             omegaVelGRF_ChSystemBackupH, omegaAccGRF_ChSystemBackupH,
...
// -------------------
        ...
        if (tStep % 2 == 0) {
            printf("step: %d, realTime: %f, step Time (CUDA): %f, step Time (CPU): %f\n ", tStep, realTime,
                   (Real)myGpuTimer.Elapsed(), 1000 * mCpuTimer.Elapsed());
        }
#endif
