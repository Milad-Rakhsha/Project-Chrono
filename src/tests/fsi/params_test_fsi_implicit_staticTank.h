...
Real fluidInitDimX = 0.3;
Real fluidInitDimY = 0.03;  // This is half of the width
Real fluidInitDimZ = 1.5;
// Dimensions
Real hdimX = 0.3;
Real hdimY = 0.03;  // Should be the same as fluidInitDimY, BCEs are taken care of later
Real hdimZ = 1.8;
...
void SetupParamsH(SimParams& paramsH) {
    ...
    paramsH.HSML = 0.03;    // 0.06;    // 0.06;//0.04;
    ...
    paramsH.gravity = mR3(0, 0, 0);  // mR3(0);//mR3(0, -9.81, 0);
    paramsH.bodyForce3 =
    ...
    paramsH.enableAggressiveTweak =
        0;  // 0: no aggressive tweak; 1: with aggressive tweak (if 1, enableTweak should be 1 too)
    paramsH.tweakMultV =
        0.1;  // paramsH.v_Max / (paramsH.HSML / paramsH.dT);//	maximum allowed velocity: tweakMultV * HSML / dT;
              // NOTE: HSML and dT must be defined. So this line comes after them
    ...
    paramsH.cMax =
        mR3(hdimX, hdimY + (3 * paramsH.HSML), hdimZ) + 2 * paramsH.NUM_BOUNDARY_LAYERS * paramsH.HSML * mR3(1, 0, 1);
    //****************************************************************************************
    ...
    paramsH.deltaPress = mR3(
        0);  // Wrong: 0.9 * paramsH.boxDims * paramsH.bodyForce3; // viscosity and boundary shape should play a roll
