...
#include "demos/fsi/demo_FSI_KitchenSink_Implicit.h"  //SetupParamsH()
...
const std::string h_file =
    "/home/milad/CHRONO/Project-Chrono-Milad-IISPH/src/demos/fsi/demo_FSI_KitchenSink_Implicit.h";
const std::string cpp_file =
    "/home/milad/CHRONO/Project-Chrono-Milad-IISPH/src/demos/fsi/demo_FSI_KitchenSink_Implicit.cpp";

const std::string out_dir = "FSI_OUTPUT";  //"../FSI_OUTPUT";
const std::string pov_dir_fluid = out_dir + "/KitchenSink/";
...
void CreateMbdPhysicalSystemObjects(ChSystemParallelDVI& mphysicalSystem,
    ...
    // Top Sink
    Real temp_y = (byDim - sink_hole_size) / 4;
    ChVector<> XY_1(bxDim / 2 + 3 * paramsH->HSML, temp_y + 1 * paramsH->HSML, 2 * paramsH->HSML);
    ChVector<> posXY_1(0, temp_y + sink_hole_size / 2 + 2 * paramsH->HSML, -2 * paramsH->HSML);
    ChVector<> XY_2(bxDim / 2 + 3 * paramsH->HSML, temp_y + 1 * paramsH->HSML, 2 * paramsH->HSML);
    ChVector<> posXY_2(0, -temp_y - sink_hole_size / 2 - 2 * paramsH->HSML, -2 * paramsH->HSML);
    Real temp_x = (bxDim - sink_hole_size) / 4;
    ChVector<> XY_3(temp_x + 1 * paramsH->HSML, (sink_hole_size) / 2, 2 * paramsH->HSML);
    ChVector<> posXY_3(temp_x + sink_hole_size / 2 + 2 * paramsH->HSML, 0, -2 * paramsH->HSML);
    ChVector<> XY_4(temp_x + 1 * paramsH->HSML, (sink_hole_size) / 2, 2 * paramsH->HSML);
    ChVector<> posXY_4(-temp_x - sink_hole_size / 2 - 2 * paramsH->HSML, 0, -2 * paramsH->HSML);
    ...
    //////////////Bottom Sink
    /////////////////////////
    ChVector<> BottomSink(bxDim / 2 + 3 * paramsH->HSML, byDim / 2 + 3 * paramsH->HSML, 2 * paramsH->HSML);
    ChVector<> posBottom(0, 0, -2 * paramsH->HSML - bzDim - 4 * paramsH->HSML);
    // left and right Wall
    ChVector<> size_YZ_b(2 * paramsH->HSML, byDim / 2 + 3 * paramsH->HSML, bzDim / 2);
    ChVector<> pos_xp_b(bxDim / 2 + paramsH->HSML, 0.0, bzDim / 2 + 1 * paramsH->HSML - bzDim - 4 * paramsH->HSML);
    ChVector<> pos_xn_b(-bxDim / 2 - 3 * paramsH->HSML, 0.0, bzDim / 2 + 1 * paramsH->HSML - bzDim - 4 * paramsH->HSML);
    // Front and back Wall
    ChVector<> size_XZ_b(bxDim / 2, 2 * paramsH->HSML, bzDim / 2);
    ChVector<> pos_yp_b(0, byDim / 2 + paramsH->HSML, bzDim / 2 + 1 * paramsH->HSML - bzDim - 4 * paramsH->HSML);
    ChVector<> pos_yn_b(0, -byDim / 2 - 3 * paramsH->HSML, bzDim / 2 + 1 * paramsH->HSML - bzDim - 4 * paramsH->HSML);

    chrono::utils::AddBoxGeometry(ground.get(), XY_1, posXY_1, chrono::QUNIT, true);
    chrono::utils::AddBoxGeometry(ground.get(), XY_2, posXY_2, chrono::QUNIT, true);
    chrono::utils::AddBoxGeometry(ground.get(), XY_3, posXY_3, chrono::QUNIT, true);
    chrono::utils::AddBoxGeometry(ground.get(), XY_4, posXY_4, chrono::QUNIT, true);
    ...
    chrono::fsi::utils::AddBoxBce(myFsiSystem.GetDataManager(), paramsH, ground, posXY_1, chrono::QUNIT, XY_1);
    chrono::fsi::utils::AddBoxBce(myFsiSystem.GetDataManager(), paramsH, ground, posXY_2, chrono::QUNIT, XY_2);
    chrono::fsi::utils::AddBoxBce(myFsiSystem.GetDataManager(), paramsH, ground, posXY_3, chrono::QUNIT, XY_3);
    chrono::fsi::utils::AddBoxBce(myFsiSystem.GetDataManager(), paramsH, ground, posXY_4, chrono::QUNIT, XY_4);
    ...
    chrono::fsi::utils::AddBoxBce(myFsiSystem.GetDataManager(), paramsH, ground, posBottom, chrono::QUNIT, BottomSink);
    chrono::fsi::utils::AddBoxBceYZ(myFsiSystem.GetDataManager(), paramsH, ground, pos_xp_b, chrono::QUNIT, size_YZ_b);
    chrono::fsi::utils::AddBoxBceYZ(myFsiSystem.GetDataManager(), paramsH, ground, pos_xn_b, chrono::QUNIT, size_YZ_b);
    chrono::fsi::utils::AddBoxBceXZ(myFsiSystem.GetDataManager(), paramsH, ground, pos_yp_b, chrono::QUNIT, size_XZ_b);
    chrono::fsi::utils::AddBoxBceXZ(myFsiSystem.GetDataManager(), paramsH, ground, pos_yn_b, chrono::QUNIT, size_XZ_b);
