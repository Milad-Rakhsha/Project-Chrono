...
void SetupParamsH(SimParams* paramsH, Real bxDim, Real byDim, Real bzDim, Real fxDim, Real fyDim, Real fzDim) {
    ...
    paramsH->BASEPRES = 1e-5;
    paramsH->LARGE_PRES = 1e-6;
    paramsH->deltaPress;
    ...
    paramsH->IncompressibilityFactor = 0.5;   // Increasing this causes lager compressibility, but let for larger dt
    ...
    paramsH->cMax = mR3(bxDim, byDim, bzDim) + mR3(paramsH->HSML * 5);
