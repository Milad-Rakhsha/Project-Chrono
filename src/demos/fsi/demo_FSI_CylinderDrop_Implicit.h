...
void SetupParamsH(SimParams* paramsH, Real bxDim, Real byDim, Real bzDim, Real fxDim, Real fyDim, Real fzDim) {
    ...
    paramsH->mu0 = 0.1;
    paramsH->kappa = 0.001;  ///< surface tension parameter, experimental
    paramsH->v_Max = 1;
    paramsH->EPS_XSPH = .5f;
    ...
    paramsH->cMin = mR3(-bxDim / 2, -byDim / 2, -bzDim / 2 - 5 * initSpace0) * 2 - 4 * initSpace0;
    paramsH->cMax = mR3(bxDim / 2, byDim / 2, bzDim + 10 * initSpace0) * 2 + 4 * initSpace0;
    ...
    paramsH->straightChannelBoundaryMin = paramsH->cMin;
    paramsH->straightChannelBoundaryMax = paramsH->cMax;
    //******************************* modify pressure ****************************************
